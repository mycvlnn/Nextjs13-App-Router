...
import { Brand, Category, Product, Property } from "@/types";
...
export const AllProductClient: React.FC<AllProductClientProps> = ({ }) => {
    ...
    const { data: categories } = useSWRImmutable<Category | null>([`${URL}/api/products/new-product/get-category`],
        (url: string) =>
        axios
            .get(url)
            .then((res) => res.data.data)
    ...
    const { data: products } = useSWR<Product[] | null>(
        [`${URL}/api/products/public-store/get-all-product-nolimit?${new URLSearchParams(searchParams).toString()}`],
        (url: string) =>
          axios
            .get(url)
            .then((res) => res.data.data)
