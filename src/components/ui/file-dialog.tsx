      ...
      if (rejectedFiles.length > 0) {
        rejectedFiles.forEach(({ errors }) => {
          if (errors[0]?.code === "file-too-large") {
            toast.error(
              `Tệp quá lớn. Kích thước tối đa là ${formatBytes(maxSize)}`,
            );
            return;
          }
          ...
          {isUploading ? (
            ...
            <div className="grid place-items-center gap-2 text-muted-foreground sm:px-5">
              <Icons.upload
                ...
                aria-hidden="true"
              />
              <p className="text-base font-medium">Kéo tệp vào đây</p>
            </div>
          ) : (
            <div className="grid place-items-center gap-1 sm:px-5">
              ...
              <p className="mt-2 text-base font-medium text-muted-foreground">
                Kéo {`'n'`} thả tệp tại đây, hoặc click vào đây để chọn
              </p>
              <p className="text-sm text-slate-500">
                Vui lòng tải lên tệp có kích thước nhỏ hơn {formatBytes(maxSize)}
              </p>
            </div>
          )}
        </div>
        <p className="text-center text-sm font-medium text-muted-foreground">
          Bạn có thể tải lên tối đa {maxFiles} {maxFiles === 1 ? "file" : "files"}
        </p>
        {files?.length ? (
  ...
  return (
    <div className="relative flex items-center justify-between gap-2.5">
      ...
      <div className="flex items-center gap-2">
          ...
          <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <DialogTrigger asChild>
              <Button
                ...
                className="h-7 w-7"
              >
                <Icons.crop className="h-4 w-4 text-muted-foreground" aria-hidden="true" />
        ...
        >
          <Icons.close className="h-4 w-4 text-muted-foreground" aria-hidden="true" />
