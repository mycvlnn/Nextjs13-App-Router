...
import { Copy, FileDown, FileSearch2 } from "lucide-react";
...
interface OrdersTableShellPublicProps {
  data: Order[];
  pageCount: number;
}

export function OrdersTableShellPublic({
  data,
  pageCount,
}: OrdersTableShellPublicProps) {
        ...
        cell: ({ row }) => (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-[220px]">
                ...
                <Link href={`/order/${row.original.code}`}>
                  <FileSearch2 className="w-4 h-4 mr-2"/> Chi tiết đơn hàng
                </Link>
              </DropdownMenuItem>
